  ...
  analysis: WeatherAnalysis,
    ...
    try {
      ...
      if (firstPart && firstPart.inlineData) {
        ...
        if (error instanceof Error) {
            throw new Error(`Failed to generate visual summary image: ${error.message}`);
        }
        throw new Error("Failed to generate visual summary image.");
    }
}

export async function refineVisualSummaryImage(
  base64ImageData: string,
  mimeType: string,
  prompt: string,
): Promise<{ base64: string, mimeType: string }> {
  ...
  try {
    ...
    if (firstPart && firstPart.inlineData) {
      ...
      throw new Error("No image was refined by the model.");
    }
  } catch (error) {
    console.error("Error refining visual summary:", error);
    if (error instanceof Error) {
        throw new Error(`Failed to refine visual summary image: ${error.message}`);
    }
    throw new Error("Failed to refine visual summary image.");
  }
}
